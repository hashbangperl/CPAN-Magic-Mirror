...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{CPAN_MAGICMIRROR_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{CPAN_MAGICMIRROR_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require CPAN::MagicMirror;

    CPAN::MagicMirror->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
cpan_magicmirror_server.pl - Catalyst Testserver

=head1 SYNOPSIS

cpan_magicmirror_server.pl [options]
